...
class Cdnjs {
	 ...
	 * @param boolean $return True to return the results, false to print the HTML scripts
				...
				if($library != false) {
					...
					foreach ($library->assets as $asset) {
						if($asset->version == $version) {
							$loaded_libs[$library->name] = 'https://cdnjs.cloudflare.com/ajax/libs/' . $library->name . '/' . $asset->version . '/' . $library->filename;
							break;
						}
					}
